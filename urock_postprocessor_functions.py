...
from urock_processing import H2gisConnection
from urock_processing.loadData import loadFile
from urock_processing import DataUtil
from urock_processing.GlobalVariables import WIND_GROUP, TEMPO_DIRECTORY,\
    RLON, RLAT, GEOM_FIELD, LON , LAT, WINDSPEED_X, WINDSPEED_Y, WINDSPEED_Z,\
    Z

idx = pd.IndexSlice

def plotSectionalViews(pluginDirectory, inputWindFile, lines_file, srid_lines,
    ...
    # Load lines needed for calculation
    ...
    for line in sorted(set(df_selectedPoints[idLines.upper()])):
        ...
        ax.set_title("Line {0}".format(line))
        ...
        if isStream:
            ...
            ax.streamplot(D, z, wind_d, wind_z, density = streamDensity)
        else:
            ...
            Q = ax.quiver(D, z, wind_d, wind_z, 
                          units = 'xy', scale = dist_max, headwidth = headwidth, 
        ...
        for i, bcell in enumerate(buildIndexes):
            ...
            ax.add_patch(rect[i])
        if savePlot:
            fig.savefig(os.path.join(outputDirectory, simulationName + str(line) + ".png"))
